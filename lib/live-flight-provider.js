...
export function normalizeTravelpayoutsHandoff(link) {
  if (!link || typeof link !== 'string') return null;
  try {
    const url = new URL(link, 'https://www.aviasales.com');
    if (!/(^|\.)aviasales\.com$/i.test(url.hostname)) return null;
    if (!/^\/search\/[A-Z]{3}\d{4}[A-Z]{3}\d{4}\d+$/.test(url.pathname)) return null;
    if (!url.searchParams.get('itinerary_key') || !url.searchParams.get('expected_price_uuid')) {
      return null;
    }
    url.protocol = 'https:';
...
export async function searchTravelpayoutsFlights({
  ...
  try {
    ...
    const flights = (Array.isArray(json?.data) ? json.data : []).flatMap(row => {
      ...
      const offerHandoff = normalizeTravelpayoutsHandoff(row.link);
      return [{
        ...
        currency: 'EUR',
        bookingUrl: offerHandoff || buildAviasalesUrl(pkg),
        bookingHandoff: offerHandoff ? 'offer_specific' : 'roundtrip_search',
