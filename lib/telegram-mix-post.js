...
function buildMixMessage(trip, tripUrl) {
  ...
  const dates = hotelOnly
    ? (h.checkIn ? ` · ${esc(h.checkIn)}${h.checkOut ? ' – ' + esc(h.checkOut) : ''}` : '')
    : (f.depart ? ` · ${esc(f.depart)}${f.return ? ' – ' + esc(f.return) : ''}` : '')
  return [
    hotelOnly ? '🏨 <b>Nov LETTO Hotel</b>' : '🪙 <b>Nov LETTO Mix</b>',
    '',
    hotelOnly ? `🏨 <b>${esc(r.dest)}</b>${dates}` : `✈ <b>${esc(r.origin)} → ${esc(r.dest)}</b>${dates}`,
