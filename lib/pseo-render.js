...
// v44-A.3 · "living compass" — real great-circle bearing + distance from the
// origin to the destination, baked at render time (no deps, no API). 8-sector
// SR/EN compass rose. The needle stroke-draws then rotates to the bearing once
// in view (gated by prefers-reduced-motion → static at bearing).
const SECTORS_SR = ['S', 'SI', 'I', 'JI', 'J', 'JZ', 'Z', 'SZ'];   // Sever, Severoistok, …
const SECTORS_EN = ['N', 'NE', 'E', 'SE', 'S', 'SW', 'W', 'NW'];
function bearingKm(lat1, lng1, lat2, lng2) {
  const toRad = d => d * Math.PI / 180, toDeg = r => r * 180 / Math.PI;
  const p1 = toRad(lat1), p2 = toRad(lat2), dl = toRad(lng2 - lng1), dp = toRad(lat2 - lat1);
  ...
  return { bearing, km, sector: (b => Math.round(b / 45) % 8)(bearing) };
}
// Compass rose with an animatable needle. `caption` is set as the aria-label.
function livingCompass(bearing, caption) {
  return `<div class="lc-wrap">
    <div class="divider">
      <span class="rule"></span>
      <div class="living-compass" data-bearing="${bearing}" style="--lc-bearing:${bearing}deg" role="img" aria-label="${esc(caption)}">
        <svg viewBox="0 0 100 100" aria-hidden="true" focusable="false">
          <defs><linearGradient id="lcg" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#7C5B22"/><stop offset="50%" stop-color="#D9A94A"/><stop offset="100%" stop-color="#7C5B22"/></linearGradient></defs>
          <circle cx="50" cy="50" r="46" fill="none" stroke="url(#lcg)" stroke-width="1"/>
          <circle cx="50" cy="50" r="39" fill="none" stroke="url(#lcg)" stroke-width="0.4" opacity="0.5"/>
          <g stroke="url(#lcg)" stroke-width="1"><line x1="50" y1="5" x2="50" y2="13"/><line x1="50" y1="87" x2="50" y2="95"/><line x1="5" y1="50" x2="13" y2="50"/><line x1="87" y1="50" x2="95" y2="50"/></g>
          <g class="lc-needle">
            <line class="lc-needle-line" x1="50" y1="50" x2="50" y2="18" stroke="#7C5B22" stroke-width="2" stroke-linecap="round"/>
            <path class="lc-needle-tip" d="M50 11 L54.5 20 L50 16.5 L45.5 20 Z" fill="#7C5B22"/>
          </g>
          <circle cx="50" cy="50" r="3" fill="url(#lcg)"/>
        </svg>
      </div>
      <span class="rule"></span>
    </div>
    <p class="lc-caption">${esc(caption)}</p>
  </div>`;
}
...
export function renderPseoPage(page) {
  ...
  // v44-A.3 · living compass for the divider ABOVE FAQ (others stay plain).
  // Origin: pSEO → the route origin; destination landing → BEG (per spec).
  let livingCompassHtml = divider();
  const lcOrigin = origin || originByCode('BEG');
  if (facts.geo && lcOrigin && lcOrigin.lat != null) {
    const bk = bearingKm(lcOrigin.lat, lcOrigin.lng, facts.geo.lat, facts.geo.lng);
    const sector = (isSr ? SECTORS_SR : SECTORS_EN)[bk.sector];
    const oGen = isSr ? lcOrigin.name_sr_gen : lcOrigin.name_en;
    const lcCaption = isSr
      ? `${city} · ${bk.bearing}° ${sector} · ${bk.km} km od ${oGen}`
      : `${city} · ${bk.bearing}° ${sector} · ${bk.km} km from ${oGen}`;
    livingCompassHtml = livingCompass(bk.bearing, lcCaption);
  }
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* v44-A.3 living compass */
  .lc-wrap { margin: 48px 0; }
  .lc-wrap .divider { margin: 0 0 10px; }
  .living-compass { width: 66px; height: 66px; flex: none; }
  .living-compass svg { width: 100%; height: 100%; overflow: visible; }
  .lc-needle { transform-box: view-box; transform-origin: 50px 50px; transform: rotate(0deg); }
  .lc-needle-line { stroke-dasharray: 32; stroke-dashoffset: 32; }
  .lc-needle-tip { opacity: 0; transform-box: view-box; transform-origin: 50px 50px; }
  .living-compass.lc-drawn .lc-needle-line { stroke-dashoffset: 0; transition: stroke-dashoffset .6s ease; }
  .living-compass.lc-drawn .lc-needle-tip { opacity: 1; transition: opacity .3s ease .4s; }
  .living-compass.lc-rot .lc-needle { transform: rotate(var(--lc-bearing)); transition: transform .9s cubic-bezier(.16,1,.3,1); }
  .lc-caption { text-align: center; font-family: 'Instrument Serif', Georgia, serif; font-style: italic; font-size: 14px; color: var(--gold-deep); margin: 0; }
  @media (prefers-reduced-motion: reduce) {
    .lc-needle-line { stroke-dashoffset: 0 !important; }
    .lc-needle-tip { opacity: 1 !important; }
    .living-compass .lc-needle { transform: rotate(var(--lc-bearing)) !important; transition: none !important; }
  }
...
(function () {
  ...
  // v44-A.3 · living compass — stroke-draw the needle, then rotate to bearing
  // once in view. prefers-reduced-motion → CSS pins it static at the bearing.
  var lc = document.querySelector('.living-compass');
  if (lc) {
    var lcGo = function () { lc.classList.add('lc-drawn'); setTimeout(function () { lc.classList.add('lc-rot'); }, 650); };
    if (reduce) { lc.classList.add('lc-drawn', 'lc-rot'); }
    else if ('IntersectionObserver' in window) {
      var io2 = new IntersectionObserver(function (es) {
        es.forEach(function (e) { if (e.isIntersecting) { lcGo(); io2.unobserve(lc); } });
