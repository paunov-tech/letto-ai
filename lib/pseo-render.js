...
export function renderPseoPage(page) {
  ...
  // priceValidUntil is computed at RENDER time (now + 30d), NOT from generatedAt.
  // Pre-gen writes generatedAt once but pages live for months; deriving validity
  // from it would expire every Offer ~30d post-pre-gen and Google would suppress
  // the Offer rich result forever. Render-time keeps the CDN-refreshed HTML fresh.
