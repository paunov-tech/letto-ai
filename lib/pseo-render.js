...
// Hero image resolution. Prefer an explicit page.hero {url, alt, credit,
// creditUrl}; else fall back to the P3 self-host convention path (gradient base
// + duotone overlay mean a missing file still looks intentional).
function resolveHero(page, dest, city, isSr) {
  if (page.hero && page.hero.url) return page.hero;
  return {
    url: `/img/dest/${dest.enSlug}-hero.jpg`,
    alt: isSr ? `${city} — panorama` : `${city} skyline`,
...
export function renderPseoPage(page) {
  ...
  const geo = facts.geo;
  const heroAbs = hero.url.startsWith('http') ? hero.url : ORIGIN + hero.url;
...
<header class="hero grain">
  <img class="hero-img" src="${esc(hero.url)}" alt="${esc(hero.alt)}" fetchpriority="high" width="1200" height="627" decoding="async">
      ...
      .then(function (data) {
        ...
        arr.slice(0, 3).forEach(function (p) {
          ...
          el.innerHTML = '<div><div class="deal-meta">' + meta + '</div>' + badge + '</div>' +
            '<div class="deal-price"><span class="amt">' + (price ? '€' + price : '—') + '</span><span class="per">' + perPkg + '</span></div>';
          box.appendChild(el);
